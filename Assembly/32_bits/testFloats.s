...
loop1:
   ...
   fdiv qword ptr r             # pop the floating point stack top (1/r), divide it over r and push the result (1/r*r)
...
mov ecx ,n 
loop2:
fld qword ptr r1                 # push 1 to the floating point stack
fadd qword ptr s1                # pop the floating point stack top (1+r1), add it to s, and push the result (s1+(1+r1))
fstp qword ptr s1                # pop the floating point stack top (s1+(1+r)) into the memory variable s1
 # the following 3 instructions increase r by 1   
   fld qword ptr r1              # push 1 to the floating point stack
   fadd qword ptr one           # pop the floating point stack top (1), add it to r and push the result (r+1)
   fstp qword ptr r1             # pop the floating point stack top (r+1) into the memory variable r
loop loop2                      # ecx -=1 , then goto loop1 only if ecx is not zero
fld qword ptr s1                #push s1 to the floating point stack
fadd qword ptr s                #pop the floating point stack top (s1), add it to s, and push the result (s1+s))
fstp qword ptr s                #pop the floating point stack top  (s1+s))into the memory variable s
